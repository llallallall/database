module.exports.setup = function (app, db) {
        app.put('/db/accounts/:email/:password/:oldpassword',(req, res, next) => {
                ...
                db.get(
                        `SELECT * FROM tbl_accounts WHERE ( email = '${req.params.email}' OR email='vue') AND grade='owner'`, (err, row) => {
                                if (!err && row) {
                                                ...
                                                db.run (
                                                        ...
                                                        (err1) => {
                                                                if (err1) {
                                                                        ...
                                                                        result.rsp = 'ok'
                                                                        result.temp = `UPDATE tbl_accounts ET email='${req.params.email}', password='${req.params.oldpassword}' WHERE (email='${req.params.email}' OR email='vue') AND grade='owner' AND password='${req.params.password}'`
                                                                        res.json(result)
                                                                }
                                                        }
                                                )
